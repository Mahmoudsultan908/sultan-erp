-- تشديد صلاحية RPC حركات رأس المال: الجلسة المسجلة فقط.
-- النص الكامل موجود في investor_movements_treasury_atomic_migration.sql
-- وتتم إعادة تطبيقه هنا بشكل idempotent لتحديث جسم الدالة في الإنتاج.
...
begin
    ...
    if p_tx_type = 'contribution' then
        ...
        perform public.post_cash('in', p_amount, v_description, 'capital_partner_transaction', v_tx_id, (select auth.uid()), p_treasury_id, p_tx_date::timestamptz);
        ...
        perform public.post_cash('out', p_amount, v_description, 'capital_partner_transaction', v_tx_id, (select auth.uid()), p_treasury_id, p_tx_date::timestamptz);
    else
    ...
    perform public.post_journal(
        'CAPITAL-TX-' || left(v_tx_id::text, 8), v_description,
        'capital_partner_transaction', v_tx_id, (select auth.uid()),
        v_journal_lines, p_tx_date::timestamptz
    );
    return v_tx_id;
end;
$$;
