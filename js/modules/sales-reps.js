...
let _repPriceLevels = []; // price_levels — عشان الأدمن يتحكم فى مستوى سعر كل مندوب
let _repTreasuries = [];  // treasuries — عرض خزنة المندوب (بتتعمل تلقائي، مش بتتضاف يدوي هنا)
...
async function renderSalesReps(c) {
    ...
    try {
        ...
        const [{ data: levels }, { data: treasuries }] = await Promise.all([
            sb.from('price_levels').select('id,code,name').order('sort_order'),
            sb.from('treasuries').select('id,name').order('name'),
        ]);
        _repPriceLevels = levels || [];
        _repTreasuries = treasuries || [];
...
window.repSave = async function () {
    ...
    try {
        let repId = _repEditingId;
        if (_repEditingId) {
            const { error } = await sb.from('sales_reps').update(payload).eq('id', _repEditingId);
            if (error) throw error;
        } else {
            const { data, error } = await sb.from('sales_reps').insert({ ...payload, is_active: true, created_by: currentUser?.id || null }).select('id').single();
            if (error) throw error;
            repId = data.id;
        }
