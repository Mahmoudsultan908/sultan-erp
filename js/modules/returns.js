...
let retPriceLevelCode = '';
// ★ "خصم من رصيد العميل؟" — Checkbox في مرتجع المبيعات (افتراضياً مفعّل).
//   بيتبعت كـ sales_returns.affects_customer_balance وقت الحفظ، وTrigger
...
async function renderReturns(c) {
    ...
    try {
        ...
        // خريطة أسعار مستويات مرتجع المبيعات: 'productId|LEVELCODE' => price
        // (نفس بناء INV_DB.priceMap في sales.js بالحرف — اختياري، لو الجداول
        // لسه مش موجودة r8/r9 بيرجعوا فاضيين والدروب داون بيختفي بهدوء)
        RET_DB.priceLevels = r8.data || [];
        RET_DB.priceMap = {};
        (r9.data || []).forEach(pp => {
            const code = pp.price_levels?.code;
            if (code) RET_DB.priceMap[pp.product_id + '|' + code] = Number(pp.price) || 0;
        });
...
window.retSave = async function () {
    ...
    const treasuryId = document.getElementById('retTreasuryId')?.value || null;
    const total = retCalcTotal();
